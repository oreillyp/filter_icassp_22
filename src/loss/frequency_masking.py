...
################################################################################
# Perceptually-inspired frequency masking loss proposed by Qin et al.
################################################################################
...
class FrequencyMaskingLoss(AuxiliaryLoss):
    ...
    def _normalize(self, x: torch.Tensor):
        if self.normalize == "peak":
            valid_range = 1.0
            return (valid_range / self.peak) * x * 0.95
        else:
            return x

    def _pad(self, x: torch.Tensor):
        """
        Avoid boundary artifacts by padding waveform so that each sample appears
    ...
    def _approximate_power_spectral_density(
        ...
        # stft_matrix_abs = torch.sqrt(torch.sum(torch.square(gain_factor * stft_matrix / self.window_size), dim=-1))
        # psd_matrix = torch.square(stft_matrix_abs)

        # option 1: add tol = 1e-12 inside square root operator
        # option 2: because matrix is immediately squared, just use torch.abs?
        # option 3: why is sqrt there if undone? and is only positive output expected?
    ...
    def _compute_loss(self, x_adv: torch.Tensor, x_ref: torch.Tensor = None):
